...
function onClick(button, state)
    if button == "left" and state == "down" then
        ...
        if isMouseInPosition(startX + selectorX - 10, startY, respc(15), respc(15)) then
            triggerServerEvent("changeDataSync", localPlayer, "a.Gamemode", nil)
            ...
            triggerServerEvent("changeDataSync", localPlayer, "a.Protected", false)
...
function addToKillFeed(killer, killed, weapon)
    if getElementData(localPlayer, "a.Gamemode") == 1 then
        local killerTeam = getElementData(killer, "a.DMTeam")
        local deadTeam = getElementData(killed, "a.DMTeam")
        local weaponId = getElementData(killer, "currentWeaponID")

        local hexColors = {
            ["Terrorists"] = "#D6B370",
            ["Counter-Terrorists"] = "#72ABCC",
        }
    
        triggerEvent("sendToEventLogger", localPlayer, hexColors[killerTeam] .. getElementData(killer, "a.PlayerName") .. "#FFFFFF megölte " .. hexColors[deadTeam] .. getElementData(killed, "a.PlayerName") .. "#FFFFFF-t (fegyver: " .. exports.a_inventory:getItemName(weaponId) .. ")")
    end
end
