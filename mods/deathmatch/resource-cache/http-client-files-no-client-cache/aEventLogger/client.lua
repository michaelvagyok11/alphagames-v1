...
local event_font = dxCreateFont("files/BebasNeueLight.otf", 15, false, "antialiased")
...
function editFeed(cmd, state)
    ...
    if not getElementData(localPlayer, "a.Gamemode") == 1 then
        exports.a_interface:makeNotification(2, "Nem vagy a megfelelő játékmódban!")
        return
    end
    ...
    if getElementData(localPlayer, "a.Gamemode") == 1 and state == "on" and bgState == false then
        exports.a_interface:makeNotification(1, "Beléptél a kill-feed szerkesztőbe!")
        bgState = true
    elseif getElementData(localPlayer, "a.Gamemode") == 1 and state == "on" and bgState == true then
        exports.a_interface:makeNotification(2, "Már a kill-feed szerkesztőben vagy!")
    elseif getElementData(localPlayer, "a.Gamemode") == 1 and state == "off" and bgState == true then
        exports.a_interface:makeNotification(1, "Kiléptél a kill-feed szerkesztőből!")
        bgState = false
    elseif getElementData(localPlayer, "a.Gamemode") == 1 and state == "off" and bgState == false then
        exports.a_interface:makeNotification(2, "Nem vagy a kill-feed szerkesztőben!")
    end
end
...
function resetFeedCMD()
    resetFeed()
    exports.a_interface:makeNotification(1, "Sikeresen visszaállítottad a kill-feeded!")
end
