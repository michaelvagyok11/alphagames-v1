...
local timeSize = {respc(200), respc(75)}
local startX, startY = sX / 2, respc(25)

local robotoBold = dxCreateFont("files/fonts/Roboto-BoldCondensed.ttf", respc(25), false, "antialiased")
local robotoBold13 = dxCreateFont("files/fonts/Roboto-BoldCondensed.ttf", respc(12), false, "cleartype")
local robotoBold12 = dxCreateFont("files/fonts/Roboto-BoldCondensed.ttf", respc(13), false, "cleartype")

function onTimeRender()
    if getElementData(localPlayer, "a.Gamemode") == 4 then
        if isPanelMoving then
            local cX, cY = getCursorPosition()
            startX, startY = cX * sX, cY * sY - respc(16)
        else
        ...
        dxDrawText(secondsToTime(seconds), startX + 1, startY + respc(25) + 1, _, _, tocolor(2, 2, 2, 200), 1, robotoBold13, "center", "center")
        dxDrawText(secondsToTime(seconds), startX, startY + respc(25), _, _, tocolor(200, 200, 200, 200), 1, robotoBold13, "center", "center")
    
        dxDrawImage(startX - respc(35) - respc(100), startY, respc(100), respc(50), ":aDeathmatch/files/img/header.png", 180, 0, 0, tocolor(65/2, 160/2, 220/2, 150))
        dxDrawText("Üldözők", startX - respc(44), startY - respc(14), _, _, tocolor(2, 2, 2, 200), 1, robotoBold13, "right", "top", false, false, false, true)
        dxDrawText("Üldözők", startX - respc(45), startY - respc(15), _, _, tocolor(200, 200, 200, 200), 1, robotoBold13, "right", "top", false, false, false, true)
        dxDrawText(chaserCount, startX - respc(55), startY + respc(25), _, _, tocolor(200, 200, 200, 200), 1, robotoBold, "right", "center", false, false, false, true)
    
        dxDrawImage(startX + respc(35), startY, respc(100), respc(50), ":aDeathmatch/files/img/header.png", 0, 0, 0, tocolor(220/2, 65/2, 65/2, 150))
        dxDrawText("Menekülők", startX + respc(46), startY - respc(14), _, _, tocolor(2, 2, 2, 200), 1, robotoBold13, "left", "top", false, false, false, true)
        dxDrawText("Menekülők", startX + respc(45), startY - respc(15), _, _, tocolor(200, 200, 200, 200), 1, robotoBold13, "left", "top", false, false, false, true)
        dxDrawText(runnerCount, startX + respc(55), startY + respc(25), _, _, tocolor(200, 200, 200, 200), 1, robotoBold, "left", "center", false, false, false, true)
    end
end
...
function onClick(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
    if button == "left" and getElementData(localPlayer, "a.Gamemode") == 4 then
        if state == "down" then
            if isMouseInPosition(startX, startY + respc(20), respc(16), respc(16)) then
                isPanelMoving = true
            end
        end
        ...
        if nval == 2 then
            ...
            blipTimer = setTimer(function()
                ...
                if isEventHandlerAdded("onClientRender", root, process3DBlip2) then
                    exports.a_interface:makeNotification(3, "Now you #E48F8Fcan't#c8c8c8 see the runner(s) for 15 seconds.")
                    removeEventHandler("onClientRender", root, process3DBlip2)
                else
                    exports.a_interface:makeNotification(3, "Now you #9BE48Fcan#c8c8c8 see the runner(s) for 15 seconds.")
                    addEventHandler("onClientRender", root, process3DBlip2)
                end
            end, 15000, 0)
