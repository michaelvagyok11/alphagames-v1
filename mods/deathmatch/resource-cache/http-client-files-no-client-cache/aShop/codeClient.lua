...
function onRender()
	...
	for i, v in ipairs(itemsInShop) do
		if i <= maxItemsShowed and (i > scrollingValue) then
			...
			if v[5] == true then
				dxDrawImage(startX + 445, startY + 45 + (35 * (i - scrollingValue - 1)) + 3 + 4, 24, 24, ":a_inventory/files/items/" .. v[6]-1 .. ".png", 0, 0, 0, tocolor(150, 150, 150, a))
			else			
...
function purchaseResponse(element, type)
	...
	elseif type == "success" then
		...
		paymentDetails = {}
		paymentIsInProgress = false
	elseif type == "failed50Hp" then
		exports.a_interface:makeNotification(2, "Nincs most erre szükséged!")
		paymentDetails = {}
		paymentIsInProgress = false
	elseif type == "failed100Hp" then
		exports.a_interface:makeNotification(2, "Nincs most erre szükséged!")
		paymentDetails = {}
		paymentIsInProgress = false
	elseif type == "failed50Armor" then
		exports.a_interface:makeNotification(2, "Nincs most erre szükséged!")
		paymentDetails = {}
		paymentIsInProgress = false
	elseif type == "failed100Armor" then
		exports.a_interface:makeNotification(2, "Nincs most erre szükséged!")
		paymentDetails = {}
		paymentIsInProgress = false
	end
