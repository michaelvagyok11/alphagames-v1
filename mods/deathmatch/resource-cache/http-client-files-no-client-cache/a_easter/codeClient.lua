...
function onRender()
	...
	dxDrawImage(startX + sizeX / 2 - respc(32), startY - respc(0) + respc(12), respc(64), respc(64), "files/img/easter.png", rotation, 0, 0, tocolor(230, 140, 140, a))
	dxDrawText("alpha", startX + sizeX / 2 - respc(42), startY - respc(0) + respc(32), _, _, tocolor(225, 140, 135, a), 1, poppinsBoldBig, "right", "center")
	...
	dxDrawText("Húsvéti", startX + sizeX / 2 + respc(42), startY - respc(0) + respc(32), _, _, tocolor(225, 140, 135, a), 1, poppinsBoldBig, "left", "center")
	...
	if isMouseInPosition(startX + respc(50), startY + sizeY - respc(65), sizeX - respc(100), respc(25)) then
		dxDrawRectangle(startX + respc(50), startY + sizeY - respc(65), sizeX - respc(100), respc(25), tocolor(225, 140, 135, a * 0.75))
	else
		dxDrawRectangle(startX + respc(50), startY + sizeY - respc(65), sizeX - respc(100), respc(25), tocolor(225, 140, 135, a * 0.5))
	end
	...
	dxDrawText("Kellemes húsvéti ünnepeket kíván az alphaGames csapata!", startX + sizeX / 2, startY + sizeY - respc(15), _, _, tocolor(150, 150, 150, a), 1, poppinsLighSmall, "center", "center")
end
...
function onClick(button, state, x, y, x2, y2, z2, clickedElement)
	if button == "left" and state == "down" then
		if panelOpened then
			if isMouseInPosition(startX + respc(50), startY + sizeY - respc(65), sizeX - respc(100), respc(25)) then
				triggerServerEvent("checkEasterEventTimer", localPlayer, localPlayer)
			end
...
function sendDataToClient(element, type)
	if element and isElement(element) and element == localPlayer and type then
		if type == "success" then
			exports.a_interface:makeNotification(1, "Sikeresen begyűjtötted a húsvéti ajándékodat. Leghamarabb 5 óra múlva tudsz újra ajándékot kérni.")
			removeEventHandler("onClientRender", root, onRender)
			panelOpened = false
			triggerServerEvent("giveGiftToPlayer", localPlayer, localPlayer)
		elseif type == "declined" then
			exports.a_interface:makeNotification(2, "Gyűjtöttél már be ajándékot az elmúlt 5 órában, gyere vissza később.")
		end
	end
end
