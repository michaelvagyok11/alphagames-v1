...
function useItem(itemId)
	...
	elseif realItemId == 383 or realItemId == 384 or realItemId == 385 then
		exports.stdma_pumpkin:TryToStartPumpkinOpening(itemsTable.player[slotId].dbID)
	...
	function (button, state, absX, absY, worldX, worldY, worldZ, hitElement)
		...
		if button == "left" then
			...
			if state == "down" then
				...
				if hoverSlotId and itemsTable[itemsTableState][hoverSlotId] then
					...
					if not itemsTable[itemsTableState][hoverSlotId].inUse then
						...
						outputChatBox("#DC143C[alphaGames]: #FFFFFFHasználatban lévő itemet nem mozgathatsz!", 255, 255, 255, true)
					end
				end
					...
					if not isPointOnActionBar(absX, absY) and not isPointOnInventory(absX, absY) then
						...
						if not isElement(hitElement) then
							...
							if dist <= 5 then
								...
								elseif isItemDroppable(movedItem.itemId) and not movedItem.dropped then
									...
									outputChatBox("#DC143C[alphaGames]: #FFFFFFEzt a tárgyat nem lehet eldobni!", 255, 255, 255, true)
									playSound("files/sounds/select.wav")
								end
							else
								outputChatBox("#DC143C[alphaGames]: #FFFFFFIlyen messze nem dobhatsz el tárgyat!", 255, 255, 255, true)
								playSound("files/sounds/select.wav")
							end
						...
						elseif elementType == "object" and isTrashModel(elementModel) then
								...
								if itemsTableState == "player" then
									if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 4 then
										...
										if tonumber(weight) >= 20 and tonumber(movedItem.itemId) == 348 then
											outputChatBox("#DC143C[alphaGames]: #FFFFFFAz inventory nem bírja el jelenlegi tárgyaid az item nélkül!", 255, 255, 255, true)
											playSound("files/sounds/select.wav")
										else
							...
							if movedItem.data3 == "duty" then
								outputChatBox("#DC143C[alphaGames]: #FFFFFFSzolgálati eszközzel ezt nem teheted meg!", 255, 255, 255, true)
								movedSlotId, haveMoving = false, false
								playSound("files/sounds/select.wav")
								return
							end
							...
							if elementType == "player" and hitElement == localPlayer and itemsTableState == "player" then
								outputChatBox("#DC143C[alphaGames]: #FFFFFFSaját inventoryból magadra nem húzhatsz itemet!", 255, 255, 255, true)
								movedSlotId, haveMoving = false, false
								playSound("files/sounds/select.wav")
								return
							end
							...
							if (itemsTableState == "vehicle" or itemsTableState == "object") and (elementType ~= "player" or hitElement ~= localPlayer) then
								outputChatBox("#DC143C[alphaGames]: #FFFFFFJárműből / széfből csak a saját inventorydba pakolhatsz!", 255, 255, 255, true)
								movedSlotId, haveMoving = false, false
								playSound("files/sounds/select.wav")
								return
							end
							...
							if tonumber(elementId) then
								...
								outputChatBox("#DC143C[alphaGames]: #FFFFFFA kiválasztott elem nem rendelkezik önálló tárterülettel!", 255, 255, 255, true)
								playSound("files/sounds/select.wav")
							end
						end
					end
			...
			if itemsTableState == "player" and isKeyItem(movedItem.itemId) and hoverSlotId < defaultSettings.slotLimit then
				...
				outputChatBox("#DC143C[alphaGames]: #FFFFFFEz az item átkerült a kulcsokhoz!", 255, 255, 255, true)
			end
			...
			if itemsTableState == "player" and isPaperItem(movedItem.itemId) and hoverSlotId < defaultSettings.slotLimit then
				...
				outputChatBox("#DC143C[alphaGames]: #FFFFFFEz az item átkerült az iratokhoz!", 255, 255, 255, true)
			end
			...
			if hoverSlotId >= defaultSettings.slotLimit * 2 then
				if not isPaperItem(movedItem.itemId) then
					if isKeyItem(movedItem.itemId) then
						...
						outputChatBox("#DC143C[alphaGames]: #FFFFFFEz nem irat!", 255, 255, 255, true)
					end
				end
			end
			...
			if hoverSlotId >= defaultSettings.slotLimit and hoverSlotId < defaultSettings.slotLimit * 2 then
				if not isKeyItem(movedItem.itemId) then
					if isPaperItem(movedItem.itemId) then
						...
						hoverSlotId = findEmptySlot("player")
						outputChatBox("#DC143C[alphaGames]: #FFFFFFEz nem kulcs item!", 255, 255, 255, true)
					end
				end
			end
				...
				if not hoverSlotId and isElement(hitElement) and hitElement ~= localPlayer and hitElement ~= currentInventoryElement then
					...
					if elementType == "vehicle" then
						...
						if getPedOccupiedVehicle(localPlayer) then
							outputChatBox("#DC143C[alphaGames]: #FFFFFFJárműben ülve nem nézhetsz bele a csomagtartóba!", 255, 255, 255, true)
							return
						end
			...
			for i = 1, #showedItems do
				...
				if v then
					...
					if availableItems[item.itemId] then
						showTooltip2(128, defaultSettings.slotBoxHeight + 16, "#d19d6b"..getItemName(item.itemId), false, item, true)
					end
				end
			end
...
function onRender()
	...
	if not guiGetInputEnabled() then 
		...
		drawOutline(panelPosX + panelWidth - 50 - 10, panelPosY, 50, 20, tocolor(209, 157, 107, 200 * alphaProgress))
	end
	...
	if currentTab ~= "crafting" then
		...
		for i = 0, defaultSettings.slotLimit - 1 do
			...
			if cx >= x and cx <= x + defaultSettings.slotBoxWidth and cy >= y and cy <= y + defaultSettings.slotBoxHeight and not renameDetails then
				slotColor = tocolor(209, 157, 107, 200 * alphaProgress)
...
function bodySearchRender()
	...
	-- ** Cím
	dxDrawText(bodyName, searchPosX + 5, searchPosY - 5, 0, searchPosY + 18, tocolor(209, 157, 107, 255), 0.5, Roboto, "left", "center", false, false, false, true)
	...
	for i = 0, defaultSettings.slotLimit - 1 do
		...
		if cx >= x and cx <= x + defaultSettings.slotBoxWidth and cy >= y and cy <= y + defaultSettings.slotBoxHeight then
			if item then
				showTooltip2(cx, cy, "#d19d6b" .. getItemName(item.itemId), false)
...
function processTooltip(x, y, text, itemId, item, showItem)
	...
	showTooltip(x, y, "#d19d6b" .. text, showItem)
end
