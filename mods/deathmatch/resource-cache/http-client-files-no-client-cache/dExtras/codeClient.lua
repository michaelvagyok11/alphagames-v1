		...
		for k = 1, 4 do
			if isCursorInBox(x + 17, sY/2 - 5+ (k-1)*35, boxSize[1] - 34, 30) then
				...
				if questions[currentQuestion][6] == k then
					setElementData(localPlayer, "a.Money", getElementData(localPlayer, "a.Money") + amount)
					outputChatBox("#E18C88[QnA]: #FFFFFFA válaszod helyes volt, nyereményed: #9BE48F" .. amount .. "$#FFFFFF.", 255, 255, 255, true)
				else
					outputChatBox("#E18C88[QnA]: #FFFFFFA válaszod helytelen volt#FFFFFF.", 255, 255, 255, true)
				end
			end
		end
	end
...
function makeCoinFlip(commandName, amountToFlip, multipler)
	if amountToFlip and multipler then
		...
		if multipler > 2 or multipler < 2 then
			outputChatBox("#E48F8F[Hiba]: #FFFFFFA legnagyobb szorzó 2.", 255, 255, 255, true)
			inProgress = false
			return
		end
		
		setElementData(localPlayer, "a.Money", getElementData(localPlayer, "a.Money") - tonumber(flipAmount))
		addEventHandler("onClientRender", root, renderCoinFlip)
		inProgress = true

		setTimer(function()
			if a1 < (255/2) then
				outputChatBox("#E18C88[Coinflip]: #FFFFFFElvesztetted a játékot. Veszteség: #E48F8F" .. flipAmount .. "#FFFFFF$.", 255, 255, 255, true)
				removeEventHandler("onClientRender", root, renderCoinFlip)
				inProgress = false
			else
				outputChatBox("#E18C88[Coinflip]: #FFFFFFSikeresen megnyerted a játékot. Nyereményed: #9BE48F" .. flipAmount*multipler .. "#FFFFFF$.", 255, 255, 255, true)
				setElementData(localPlayer, "a.Money", getElementData(localPlayer, "a.Money") + tonumber(flipAmount*multipler))
				removeEventHandler("onClientRender", root, renderCoinFlip)
				inProgress = false
			end
		end, 5000, 1)
		...
		outputChatBox("#8FC3E4[Használat]: #FFFFFF/" .. commandName .. " [mennyiség] [szorzó]", 255, 255, 255, true)
	end
end
