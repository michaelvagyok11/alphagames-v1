...
local Roboto = dxCreateFont(":dFonts/fonts/Barlow-Regular.ttf", 22, false, "cleartype")
...
addEvent("giveItemToClient", true)
addEventHandler("giveItemToClient", getRootElement(),
	...
	if panelState ~= state then
		if state then
			...
			addEventHandler("onClientRender", getRootElement(), function ()
				...
				alphaProg = (currentTick - lastOpenTick) / 150
...
function useItem(itemId)
	...
	-- ** Halloween-i tök nyitás.
	elseif realItemId == 62 then
		triggerServerEvent("äłÄÍÄ<đ\|Ä\|ÄäđĐ$äđĐ", localPlayer, localPlayer, "dbID", item.dbID, 1, "octansSeeMTA")
			...
			if not weaponInUse then
				...
				if availableItems[weaponInUse.itemId] then
					...
					elseif weaponInUse.itemId == 155 then
						...
						setElementData(localPlayer, "currentWeaponID", weaponInUse.itemId)
			...
			if state == "up" then
				...
				if hoverSlotId and itemsTable.player[hoverSlotId] then
					...
					if item.amount ~= 1 then
						exports.stdma_hud:showInfobox("e", "Stackelt itemet nem lehet elnevezni.")
						return
					end

					if item.data3 == "duty" then
						exports.stdma_hud:showInfobox("e", "Duty itemet nem lehet elnevezni.")
						return
					end

					if item.itemId == 61 or item.itemId == 62 or item.itemId == 63 or item.itemId == 150 then
						exports.stdma_hud:showInfobox("e", "Ezt az itemet nem lehet elnevezni.")
						return
					end
					...
					if itemsTableState == "player" then
						...
						if hoverSlotId then
							if movedItem.itemId == 315 then
								exports.stdma_hud:showInfobox("e", "Névcédulát nem helyezhetsz az actionbarra.")
								...
								if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 4 then
									if getElementData(hitElement, "isSellPed") then
										if not getElementData(localPlayer, "selling_active") then 
											...
											exports.aInfobox:makeNotification(2, "Jelenleg folyamatban van egy cseréd!")
										end
									end
								end
							end
				...
				if movedItem.itemId == hoverItem.itemId and isItemStackable(hoverItem.itemId) then
					if stackAmount >= 0 then
						if movedItem.nameTag or hoverItem.nameTag then
							exports.stdma_hud:showInfobox("e", "Elnevezett itemet nem lehet stackelni.")
							...
							if isKeyItem(itemsTable[itemsTableState][hoverSlotId].itemId) and itemsTable[itemsTableState][hoverSlotId].itemId ~= 318 and KeyItemActivatedIn then
								...
								if not getElementData(localPlayer, "keyResponse") then 
									if KeySlotId then
										...
										setElementData(localPlayer, "keyResponse", true)
...
local poppinsLightBig = dxCreateFont(":dFonts/fonts/Barlow-Bold.ttf", respc(12), false, "cleartype")
...
function onRender()
	...
	dxDrawFadeRectangle(panelPosX - respc(4), panelPosY - respc(5), panelWidth, panelHeight, {25, 25, 25, 255 * alphaProgress}) -- bal
	dxDrawRoundedRectangle(panelPosX - respc(4), panelPosY - respc(5), panelWidth, panelHeight, 3, tocolor(35, 35, 35, 255 * alphaProgress)) -- bal
	...
	dxDrawImage(panelPosX, panelPosY - 4, 26, 26, ":dAccount/files/img/logo.png", 0, 0, 0, tocolor(255, 255, 255, 255 * alphaProgress))
    ...
    dxDrawText("TÁRGYAK", panelPosX + respc(25), panelPosY, panelPosX + panelWidth, panelPosY + respc(20) - 5, tocolor(200, 200, 200, 255 * alphaProgress), 1, poppinsLightBig, "left", "center", false, false, false, true)
		...
		for i = 0, defaultSettings.slotLimit - 1 do
			...
			if cx >= x and cx <= x + defaultSettings.slotBoxWidth and cy >= y and cy <= y + defaultSettings.slotBoxHeight and not renameDetails then
				...
				if item and not movedSlotId then
					if not renameProcess then
						processTooltip(cx + respc(5), cy + respc(5), getItemName(item.itemId), item.itemId, item, false)
					else
			...
			if isMouseInPosition(x, y, defaultSettings.slotBoxWidth, defaultSettings.slotBoxHeight) then
				dxDrawRectangle(x, y, defaultSettings.slotBoxWidth, defaultSettings.slotBoxHeight, tocolor(55, 55, 55, 200 * alphaProgress))
			else
				dxDrawRectangle(x, y, defaultSettings.slotBoxWidth, defaultSettings.slotBoxHeight, tocolor(45, 45, 45, 200 * alphaProgress))
			end
...
local itemlistWidth = respc(500)
local itemlistHeight = respc(600)
...
local barlowRegularSmall = dxCreateFont(":dFonts/fonts/Barlow-Regular.ttf", respc(12), false, "cleartype")
...
function itemlistCharacter(character)
	if getTickCount() < openTick + 1000 then
		return
	end
...
function itemlistRender()
	...
	if isCursorShowing() then
		...
		if isMouseInPosition(itemlistPosX + respc(5), itemlistPosY + itemlistHeight + respc(25), itemlistWidth - respc(10), respc(30)) then
	...
	dxDrawRoundedRectangle(itemlistPosX, itemlistPosY, itemlistWidth, itemlistHeight, 3, tocolor(35, 35, 35, 255))
	dxDrawRectangle(itemlistPosX + respc(5), itemlistPosY + respc(5), itemlistWidth - respc(10), respc(30), tocolor(45, 45, 45, 255))
	dxDrawText(itemlistText, itemlistPosX + respc(10), itemlistPosY + respc(5) + respc(15), _, _, tocolor(255, 255, 255), 1, barlowBoldSmall, "left", "center")

	pulseAlpha = interpolateBetween(100, 0, 0, 255, 0, 0, getTickCount()/1500, "SineCurve")
	dxDrawRectangle(itemlistPosX + respc(12) + dxGetTextWidth(itemlistText, 1, barlowBoldSmall), itemlistPosY + respc(10), respc(1), respc(20), tocolor(100, 100, 100, pulseAlpha))

	local oneSize = defaultSettings.slotBoxHeight / 0.915

	for i = 1, 14 do
		local y = itemlistPosY + 46 + (i - 1) * (oneSize+respc(2))

		dxDrawRectangle(itemlistPosX, y, itemlistWidth, oneSize, tocolor(25, 25, 25, 255))
		...
		if item then
			...
			dxDrawText("#8FC3E4(" .. item .. ") #c8c8c8" .. availableItems[item][1], itemlistPosX + oneSize + respc(5), y + oneSize / 2, _, _, tocolor(200, 200, 200, 255), 1, barlowRegularSmall, "left", "center", false, false, false, true)
		end
	end

	if #itemlistTable > 14 then
		dxDrawRectangle(itemlistPosX + itemlistWidth - 5, itemlistPosY + respc(46), 2, respc(504), tocolor(0, 0, 0, 100))
		dxDrawRectangle(itemlistPosX + itemlistWidth - 5, itemlistPosY + respc(46) + (respc(504) / #itemlistTable) * itemlistOffset, 2, (respc(504) / #itemlistTable) * 14, tocolor(155, 230, 140, 150))
	end

	if isMouseInPosition(itemlistPosX + respc(5), itemlistPosY + itemlistHeight + respc(25), itemlistWidth - respc(10), respc(30)) then
		dxDrawRectangle(itemlistPosX + respc(5), itemlistPosY + itemlistHeight + respc(25), itemlistWidth - respc(10), respc(30), tocolor(230, 140, 140, 255*0.75))
		dxDrawText("Bezárás", itemlistPosX + itemlistWidth / 2, itemlistPosY + itemlistHeight + respc(25) + respc(15), _, _, tocolor(255, 255, 255, 255), 1, barlowBoldSmall, "center", "center")
	else
		dxDrawRectangle(itemlistPosX + respc(5), itemlistPosY + itemlistHeight + respc(25), itemlistWidth - respc(10), respc(30), tocolor(230, 140, 140, 255*0.5))
		dxDrawText("Bezárás", itemlistPosX + itemlistWidth / 2, itemlistPosY + itemlistHeight + respc(25) + respc(15), _, _, tocolor(200, 200, 200, 255), 1, barlowBoldSmall, "center", "center")
	end
end

local topTexture = dxCreateTexture("files/featherTop.dds", "dxt5", true, "clamp")
local sideTexture = dxCreateTexture("files/feather.dds", "dxt5", true, "clamp")
local cornerTexture = dxCreateTexture("files/corner.dds", "dxt5", true, "clamp")
