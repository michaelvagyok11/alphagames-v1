...
function onClick(button, state)
	if button == "left" and state == "down" then
		if isMouseInPosition(startX + 200, startY + sizeY - 10, sizeX - 400, 25) then
			local inputText = exports.dDx:dxGetEditText("a.NameInput")
			if (inputText) then
				if string.len(inputText) <= 3 then
					exports.dInfobox:makeNotification(2, "Minimum 4 karakterből álljon a neved.")
				else
					inputText = inputText:gsub("%p", "")
					triggerServerEvent("changePlayerName", localPlayer, localPlayer, tostring(inputText))
					exports.dInfobox:makeNotification(1, "Sikeresen beállítottad a nevedet. #9BE48F(" .. inputText .. ")")
					removeEventHandler("onClientRender", root, onRender)
					removeEventHandler("onClientClick", root, onClick)
					exports.dDx:dxDestroyEdit("a.NameInput")
					setElementFrozen(localPlayer, false)
					setElementAlpha(localPlayer, 255)
				end
			else
				exports.dInfobox:makeNotification(2, "Mielőtt továbblépsz, add meg a nevedet.")
			end
		end
	end
end
...
function showNamePanel(element)
	if (element == localPlayer) then
		...
		exports.dDx:dxCreateEdit("a.NameInput", "", "Megjelenített neved", startX + 50, startY + 125, sizeX - 100, 30, {125, 125, 125}, 12, 1)
