...
local RubikFont = false
local RubikHeight = false
...
addCommandHandler("debugon",
	function ()
		if getElementData(localPlayer, "adminLevel") >= 3 or getElementData(localPlayer, "acc.rpGuard") >= 3 then
			if not panelState then
				RubikFont = dxCreateFont("Rubik.ttf", 14, false, "proof")
				RubikHeight = dxGetFontHeight(0.75, RubikFont)

				panelHeight = RubikHeight * 20
...
addCommandHandler("debugoff",
	function ()
		if panelState then
			...
			if isElement(RubikFont) then
				destroyElement(RubikFont)
			end
	...
	if panelState then
		...
		for i = 1, 20 do
			if debugLines[i] then
				local y = panelPosY + panelHeight - RubikHeight * (i - 1)

				dxDrawBorderText(debugLines[i][1] .. "x", panelPosX, y, 0, 0, tocolor(255, 255, 255), tocolor(0, 0, 0, 200), 0.75, RubikFont, "left", "top")
				
				dxDrawBorderText(debugLines[i][2], panelPosX + dxGetTextWidth(debugLines[i][1] .. "x", 0.75, RubikFont) + 5, y, 0, 0, debugLines[i][3], tocolor(0, 0, 0, 200), 0.75, RubikFont, "left", "top")
			end
		end
	end
end
