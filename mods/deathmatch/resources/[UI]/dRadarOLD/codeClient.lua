...
function onRender()
	...
	if renderMinimap then
		...
		dxSetBlendMode("modulate_add")
			...
			dxDrawImage(x - 800, y - 800, 1600, 1600, "files/img/map.png", camZ, (playerX / (6000 / 1600)), -(playerY / (6000 / 1600)), tocolor(255, 255, 255, 255))
	...
	if renderF11 then
		...
		-- ** BORDER
		dxDrawRectangle(startX - 2, startY - 2, sizeX + 4, sizeY + 4, tocolor(65, 65, 65, 255))
		dxDrawRectangle(startX - 1, startY - 1, sizeX + 2, sizeY + 2, tocolor(35, 35, 35, 255))
		...
		dxSetRenderTarget(f11Target, true)
			dxDrawRectangle(0, 0, sizeX, sizeY, tocolor(110, 158, 206))
			dxDrawImage(x - (1600 * zoomState)/2, y - (1600 * zoomState)/2, 1600 * zoomState, 1600 * zoomState, "files/img/map.png", 0, (playerX / (6000 / 1600 * zoomState)), -(playerY / (6000 / 1600 * zoomState)), tocolor(255, 255, 255, 255))
			dxDrawImage(0, 0, sizeX, sizeY, "files/img/vignette.png", 0, 0, 0, tocolor(200, 200, 200, 255))
			...
			for k, v in ipairs(getElementsByType("blip")) do
				...
				dxDrawImage(blipX - 8, blipY - 8, 16, 16, "files/img/blips/1.png", 0, 0, 0, tocolor(255, 255, 255, 255))
			end
		dxSetRenderTarget()

		-- ** FOOTER
		dxDrawRectangle(startX, startY + sizeY - 30, sizeX, 30, tocolor(25, 25, 25, 255), true)
		dxDrawImage(startX + 5, startY + sizeY - 23, 15, 15, "files/img/location.png", 0, 0, 0, tocolor(200, 200, 200, 255), true)
		dxDrawText(getZoneName(Vector3(getElementPosition(localPlayer))), startX + 30, startY + sizeY - 15, _, _, tocolor(200, 200, 200, 255), 1, robotoBold, "left", "center", false, false, true)

		-- ** PLAYER ARROW
		dxDrawImage(startX + sizeX / 2 - 8, startY + sizeY / 2 - 8, 16, 16, "files/img/arrow.png", -getPedRotation(localPlayer), 0, 0, tocolor(200, 200, 200, 255), true)
