...
local rectangleWidth, rectangleHeight = panelSize[1], 25
    ...
    setTimer(function()
        for k, v in ipairs(getElementsByType("player")) do
            if getElementData(v, "a.Gamemode") == 4 then
                ...
                if pedVeh then
                    enableCollision(pedVeh)
                    fixVehicle(pedVeh)
                    triggerServerEvent("changeDataSync", pedVeh, "alphaSettings", 255)
                end
            end
        end
    end, 20000, 1)
...
function enableCollision(element)
    if getElementType(element) == "player" then
        for k, v in ipairs(getElementsByType("player")) do
            ...
            triggerServerEvent("restoreAlpha", localPlayer, element)
        end
    elseif getElementType(element) == "vehicle" then
        for k, v in ipairs(getElementsByType("vehicle")) do
            setElementCollidableWith(v, element, true)
...
function process3DBlip()
    ...
    for k, v in ipairs(getElementsByType("player")) do
        if getElementData(v, "a.HPTeam") == 2 then
            ...
            if x and y then
                text = getElementData(v, "a.PlayerName")
                textwidth = dxGetTextWidth(removeHex(text, 6), 1, robotoBold12)

                dxDrawText(getElementData(v, "a.PlayerName"), x, y, _, _, tocolor(200, 200, 200, 255), 1, robotoBold12, "center", "center", false, false, false, true)
...
function onPursuitPanelRender()
    ...
    if (isCursorShowing() and isMoving) then
		...
		startX = cursorX - movingOffsetX;
		startY = cursorY - movingOffsetY;
	end
    ...
    dxDrawRectangle(startX + 7, startY + 35, panelSize[1]/2 - 14, panelSize[2] - 45, tocolor(25, 25, 25, a))
    dxDrawRectangle(startX + 15, startY + 40, panelSize[1]/2 - 14 - 16, 30, tocolor(45, 45, 45, a))
    dxDrawText("Járművek", startX + 12 + (panelSize[1]/2 - 14 - 16)/2, startY + 40 + 30/2, _, _, tocolor(200, 200, 200, a), 1, robotoBold12, "center", "center", false, false, false, true)
    
    dxDrawRectangle(startX + 7 + panelSize[1]/2, startY + 35, panelSize[1]/2 - 14, panelSize[2] - 45, tocolor(25, 25, 25, a))
    dxDrawRectangle(startX + 7 + panelSize[1]/2 + 8, startY + 40, panelSize[1]/2 - 14 - 16, 30, tocolor(45, 45, 45, a))
    dxDrawText("Skinek", startX + 9 + panelSize[1]/2 + 8 + (panelSize[1]/2 - 14 - 16)/2, startY + 40 + 30/2, _, _, tocolor(200, 200, 200, a), 1, robotoBold12, "center", "center", false, false, false, true)
    ...
    for k, v in ipairs(vehiclesToBuy) do
        if k <= maxVehiclesShowing and (k > vehiclesScrolling) then
            if isMouseInPosition(startX + 15, startY + 40 + (k-vehiclesScrolling)*35, panelSize[1]/2 - 14 - 16, 30) then
                dxDrawRectangle(startX + 15, startY + 40 + (k-vehiclesScrolling)*35, panelSize[1]/2 - 14 - 16, 30, tocolor(50, 50, 50, a))
            else
                dxDrawRectangle(startX + 15, startY + 40 + (k-vehiclesScrolling)*35, panelSize[1]/2 - 14 - 16, 30, tocolor(40, 40, 40, a))
            end

            dxDrawRectangle(startX + panelSize[1]/2 - 45, startY + 40 + (k-vehiclesScrolling)*35 + 5, 20, 20, tocolor(75, 75, 75, a))
    ...
    for k, v in ipairs(skinsToBuy) do
        if k <= maxSkinsShowing and (k > skinsScrolling) then
            if isMouseInPosition(startX + 7 + panelSize[1]/2 + 10, startY + 40 + (k-skinsScrolling)*35, panelSize[1]/2 - 14 - 16, 30) then
                dxDrawRectangle(startX + 7 + panelSize[1]/2 + 10, startY + 40 + (k-skinsScrolling)*35, panelSize[1]/2 - 14 - 16, 30, tocolor(50, 50, 50, a))
            else
                dxDrawRectangle(startX + 7 + panelSize[1]/2 + 10, startY + 40 + (k-skinsScrolling)*35, panelSize[1]/2 - 14 - 16, 30, tocolor(40, 40, 40, a))
            end

            dxDrawRectangle(startX + panelSize[1]/2 + panelSize[1]/2 - 45, startY + 40 + (k-skinsScrolling)*35 + 5, 20, 20, tocolor(75, 75, 75, a))
                ...
                if isMouseInPosition(startX + panelSize[1]/2 - 45, startY + 40 + (k-vehiclesScrolling)*35 + 5, 20, 20) then
                    if v[4] == true then
                        if getElementData(localPlayer, "a.VIP") == true then
                            ...
                            exports.a_interface:makeNotification(2, "Ez a jármű csak VIP játékosoknak elérhető!")
                        end
                    else
                        if v[3] == "chaser" then
                            ...
                            if isValueInTable(boughtVehicles, v[1], k) then
                                ...
                                expprintorts.a_interface:makeNotification(2, "Előbb meg kell venned az autót!")
                            end
                        else
                            local boughtVehicles = getElementData(localPlayer, "a.HPBoughtVehs")
                            if isValueInTable(boughtVehicles, v[1], k) then
                                ...
                                exports.a_interface:makeNotification(2, "Előbb meg kell venned az autót!")
                            end
                        end
                    end
                end
                if isMouseInPosition(startX + panelSize[1]/2 - 90, startY + 40 + (k-vehiclesScrolling)*35, 40, 20) then
                    ...
                    if not (isValueInTable(boughtVehicles, v[1], k)) and v[4] == false then
                        ...
                        if playerMoney >= v[5] then
                            exports.a_interface:makeNotification(1, "Sikeresen megvetted a kiválasztott autót: " .. v[2] .. ".")
                            triggerServerEvent("requestSave", localPlayer, localPlayer, "buyveh", v[1])
                            triggerServerEvent("changeDataSync", localPlayer, "a.Money", playerMoney - v[5])
                        else
                            exports.a_interface:makeNotification(2, "Nincs elég pénzed ehhez!")
                        end
                    end
                end
            end
            ...
            if k <= maxSkinsShowing and (k > skinsScrolling) then
                if isMouseInPosition(startX + panelSize[1]/2 + panelSize[1]/2 - 45, startY + 40 + (k-skinsScrolling)*35 + 5, 20, 20) then
                    if v[4] == true and getElementData(localPlayer, "a.VIP") == false then
                        exports.a_interface:makeNotification(2, "Ez a kinézet csak a VIP játékosoknak elérhető!")
                    else
                        if v[3] == "chaser" then
                            currentSelectedSkin[1] = k
...
function onClick(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
	...
	if isPursuitPanelRendered == true then
		if (button == 'left' and state == 'down') then
			if (cursorX >= startX and cursorX <= startX + rectangleWidth and cursorY >= startY and cursorY <= startY + rectangleHeight) then
				isMoving = true;
				movingOffsetX = cursorX - startX;
				movingOffsetY = cursorY - startY;
			end
		else
			isMoving = false;
		end
...
function addToFeed(killer, killed)
    if getElementData(localPlayer, "a.Gamemode") == 4 then
        local killerTeam = getElementData(killer, "a.HPTeam")
        local deadTeam = getElementData(killed, "a.HPTeam")
        ...
        triggerEvent("sendToEventLogger", localPlayer, hexColors[killerTeam] .. getElementData(killer, "a.PlayerName") .. "#FFFFFF megölte " .. hexColors[deadTeam] .. getElementData(killed, "a.PlayerName") .. "#FFFFFF-t")
    end
end
addEvent("addToFeed", true)
addEventHandler("addToFeed", getRootElement(), addToFeed)