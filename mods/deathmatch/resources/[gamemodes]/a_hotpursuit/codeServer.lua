...
function restartRound(sourcePlayer, commandName, ...)
    if getElementData(sourcePlayer, "adminLevel") >= 5 then
        ...
        local adminName = getElementData(sourcePlayer, "a.PlayerName")
...
function startRound()
    ...
    for k, v in pairs(getElementsByType("player")) do 
        if getElementData(v, "a.Gamemode") == 4 then 
            --outputChatBox(getElementData(v, "a.PlayerName") .. ":true")
...
function sortPlayerIntoTeam(p)
    if isElement(p) then
        ...
        if runnerCount == 0 then
            ...
            setElementDimension(p, 4)
            outputChatBox("#5AB1D9► Hot Pursuit: #FFFFFFA rendszer automatikusan kiválasztotta a csapatodat. Te egy #E48F8Fmenekülő#FFFFFF vagy, menj és menekülj el a #8FC3E4rendőrök#FFFFFF elől.", p, 255, 255, 255, true)
            ...
            if (playerCount % 5) == 0 then
                ...
                setElementInterior(p, 0)
                setElementDimension(p, 4)
                outputChatBox("#5AB1D9► Hot Pursuit: #FFFFFFA rendszer automatikusan kiválasztotta a csapatodat. Te #8FC3E4rendőr#FFFFFF vagy, menj és kapd el a #E48F8Fmenekülőket#FFFFFF.", p, 255, 255, 255, true)
                setElementData(p, "a.Protected", false)
...
function onDeath(ammo, attacker)
    if getElementData(source, "a.HPTeam") == 1 then
        ...
        killer = getElementData(attacker, "a.PlayerName")
        killed = getElementData(source, "a.PlayerName")

        triggerClientEvent("addToFeed", killer, killed)
        --setTimer(function()
            if runnerCount == 0 then
                ...
                for k, v in ipairs(getElementsByType("player")) do
                    if getElementData(v, "a.Gamemode") == 4 then
                        if getElementData(v, "a.HPTeam") == 2 then
                            outputChatBox("#5AB1D9► Hot Pursuit: #FFFFFFA #8FC3E4rendőrök #FFFFFFnyertek.", v, 255, 255, 255, true)
                        else
                            outputChatBox("#5AB1D9► Hot Pursuit: #FFFFFFA #8FC3E4rendőrök #FFFFFFnyertek.", v, 255, 255, 255, true)
                        end
                    end
        ...
        --setTimer(function()
            if chaserCount == 0 then
                ...
                for k, v in ipairs(getElementsByType("player")) do
                    if getElementData(v, "a.Gamemode") == 4 then
                        if getElementData(v, "a.HPTeam") == 1 then
                            outputChatBox("#5AB1D9► Hot Pursuit: #FFFFFFA #8FC3E4menekülők #FFFFFFnyertek.", v, 255, 255, 255, true)
                        else
                            outputChatBox("#5AB1D9► Hot Pursuit: #FFFFFFA #8FC3E4menekülők #FFFFFFnyertek.", v, 255, 255, 255, true)
                        end
                    end
