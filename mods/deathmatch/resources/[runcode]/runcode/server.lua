function runString (commandstring, outputTo, source)
	...
	outputChatBoxR(sourceName.." executed command: "..commandstring, source, true)
	local notReturned
	...
	if errorMsg then
		...
		outputChatBoxR("Error: "..errorMsg, source)
		return
	end
	...
	if not results[1] then
		--It failed.
		outputChatBoxR("Error: "..results[2], source)
		return
	end
	...
	if #results > 1 then
		outputChatBoxR("Command results: " ..resultsString, source)
		return
	end

	outputChatBoxR("Command executed!", source)
end
