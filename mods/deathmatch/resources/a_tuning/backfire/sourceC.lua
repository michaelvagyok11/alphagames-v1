...
addEventHandler("onBackFire", getRootElement(), 
	function()
		if isElement(source) and getElementData(source, "a.Backfire") == 1 then
			if getElementData(source, "a.Backfire") == 0 then 
      			return 
      		end
			local s = playSound3D("backfire/sound/" .. math.random(1,12) .. ".wav", getElementPosition(source))
...
addEventHandler("onClientResourceStart", resourceRoot,
	function()
		fireTex = dxCreateTexture("backfire/images/fire.dds", "dxt3")
		sparkTex = dxCreateTexture("backfire/images/spark.dds", "dxt3")
	end
)
...
function processBackfire()
    	...
    	if vehBackfires[k] then
      		if isElement(vehBackfires[k][2]) then
      			if getPedOccupiedVehicle(localPlayer) then
	      			if not getElementData(getPedOccupiedVehicle(localPlayer), "a.Backfire") then 
	      				return 
	      			end
	      		end
	...
	if getPedOccupiedVehicle(localPlayer) and getPedOccupiedVehicleSeat(localPlayer) == 0 then
		if getPedOccupiedVehicle(localPlayer) ~= vehGearSecond then
			...
			if getElementData(getPedOccupiedVehicle(localPlayer), "a.Backfire") ~= 1 then
				vehicleTuning = 0
			end
		end
