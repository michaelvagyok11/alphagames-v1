...
createObjectPreview(element object, float rotX rotY, rotZ, projPosX, projPosY, projSizeX, projSizeY [, bool isRelative = false, postGUI = false, isSecRT = true, isAttached = false])
...
setVisible(element objectPreviewElement, bool isVisible)
	Set if element should be drawn to screen
isVisible(element objectPreviewElement)
	Returns above value
setAttached(element objectPreviewElement, bool isAttached)
	Set if the element position and rotation should be not handled by object_preview (if it is attached to another previewed object)
createTextureReplace(element objectPreviewElement, element texture, string textureName)
	Replace texture of previewed object, returns texture replace id
setTextureReplaceTexture(integer texture replace id,  element texture)
	Set different texture for already replaced one
destroyTextureReplace(integer texture replace id)
