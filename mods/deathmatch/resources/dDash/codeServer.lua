		...
		if (#result > 0) then
			for k, v in ipairs(result) do
				...
				if (#result > 0) then
					for k, v in ipairs(result) do
						--for i = 0, 10 do
							--table.insert(playerGroupMembers, localPlayer)
...
function addPlayerToGroup(element, target)
	if isElement(element) and isElement(target) then
		db = dbExec(connection, "INSERT INTO groupMembers SET name=?, accid=?, groupid=?", getPlayerName(target), getElementData(target, "a.accID"), getElementData(element, "a.PlayerGroup"))
...
function requestServersideSave(element, argument, ...)
	if isElement(element) and argument then
		...
		elseif argument == "createclan" then
			...
			if (#result > 0) then
				triggerClientEvent("clanCreatingResponse", element, element, "nameAlreadyExists")
				return
			else
				local query = dbQuery(connection, "SELECT * FROM groups WHERE groupSyntax=?", args[2])
				local result = dbPoll(query, -1)
				if (#result > 0) then
					triggerClientEvent("clanCreatingResponse", element, element, "syntaxAlreadyExists")
					return
				else
					...
					if (db) then
						--setElementData(element, "a.Money", getElementData(element, "a.Money") - 250000)
						triggerClientEvent("clanCreatingResponse", element, element, "success")
						...
						dbExec(connection, "INSERT INTO groupMembers SET name = ?, accid = ?, groupid = ?", getPlayerName(element), getElementData(element, "a.accID"), getElementData(element, "a.PlayerGroup"))
					end
				end
			end
		end
