...
function onRender()
	...
	if seconds == 0 then
		...
		if a == 0 then
			local respawnPos = getElementData(localPlayer, "a.RespawnPosition") or {2103.662109375, -103.2578125, 2.2428879737854}
...
function killMyself()
	if getElementData(localPlayer, "a.Gamemode") == false or getElementData(localPlayer, "a.Gamemode") == nil then
		outputChatBox("#E48F8Falphav2 ► Hiba: #FFFFFFNem tudod használni ezt a parancsot a lobbyban. Ha vissza szeretnél térni a spawnra, használd a #F1DB76/lobby #FFFFFFparancsot.", 255, 255, 255, true)
		return
	end
