local dbname = "s8878_items";
local host = "maria.srkhost.eu";
local username = "u8878_y5VtfAgrot";
local password = "NikQJ2xDCkqDcK0vN.7^IrYK";
		...
		if dataName == "canisterInHand" then
			if getElementData(source, "canisterInHand") then
				...
				if isElement(playerItemObjects[source]) then
					exports.dboneattach:attachElementToBone(playerItemObjects[source], source, 12, 0, 0.025, -0.025, 0, 270, 180)
				end
			else
				if playerItemObjects[source] then
					exports.dboneattach:detachElementFromBone(playerItemObjects[source])
		...
		elseif dataName == "usingGrinder" then
			if getElementData(source, "usingGrinder") then
				...
				if isElement(playerItemObjects[source]) then
					...
					exports.dboneattach:attachElementToBone(playerItemObjects[source], source, 12, 0, 0.05, 0.025, 0, 270, 90)
				end
			else
				if playerItemObjects[source] then
					exports.dboneattach:detachElementFromBone(playerItemObjects[source])
		...
		elseif dataName == "fishingRodInHand" then
			if getElementData(source, "fishingRodInHand") then
				...
				if isElement(playerItemObjects[source]) then
					setElementInterior(playerItemObjects[source], getElementInterior(source))
					setElementDimension(playerItemObjects[source], getElementDimension(source))
					exports.dboneattach:attachElementToBone(playerItemObjects[source], source, 12, 0.05, 0.05, 0.05, 0, -90, 0)
	...
	function (casetteMoney)
		if isElement(source) and client and client == source then
			...
			--else
				...
				--exports.dhud:showInfobox(source, "error", "Nincs nálad a pénzkazetta kinyitásához megfelelő tárgy!")
			--end
		end
	end
)
    ...
    function(localPlayer, cmd, target)
        if getElementData(localPlayer, "adminLevel") >= 1 then
            if not (target) then
                ...
                target = exports.dcore:findPlayer(localPlayer, target)
...
function useItem(item, additional, additional2)
	...
	if isElement(source) and client and client == source then 
		...
		elseif itemId == 42 then
			...
			if getElementData(source, "adminLevel") <= 5 then
				if specialItemsTable[winnedItem[1]] then
					...
					outputChatBox("#D9B45A► alphaGames: #5AB1D9".. getPlayerName(source) .."#ffffff nyitott egy #D9B45A"..winnedItemName.."#ffffff tárgyat.", root, 255, 255, 255, true)
				else
					outputChatBox("#D9B45A► alphaGames: #5AB1D9".. getPlayerName(source) .."#ffffff nyitott egy #5AB1D9"..winnedItemName.."#ffffff tárgyat.", root, 255, 255, 255, true)
				end
			end
		...
		elseif itemId == 62 then
			takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
			exports.aPumpkin:TryToStartPumpkinOpening(itemsTable.player[slotId].dbID)
		elseif itemId == 58 then
			if spamTick[source] and getTickCount() - spamTick[source] < 5000 then 
				return outputChatBox("#E18C88► alphaGames: #ffffffCsak 5 másodpercenként használhatod.", source, 255, 255, 255, true)
			end
			...
			if specialItemsTable[winnedItem[1]] then
				--print(specialItemsTable[winnedItem[1]])
				outputChatBox("#D9B45A[alphaGames - Summer Event]: #5AB1D9".. getPlayerName(source) .."#ffffff nyitott egy #D9B45A"..winnedItemName.."#ffffff tárgyat.", root, 255, 255, 255, true)
				exports.dLogs:createDCLog("**"..getPlayerName(source).. "** nyitott egy Strandlabdát, ezt kapta belőle: **".. winnedItemName .."**.", 1)
			else
				outputChatBox("#D9B45A[alphaGames - Summer Event]: #5AB1D9".. getPlayerName(source) .."#ffffff nyitott egy #5AB1D9"..winnedItemName.."#ffffff tárgyat.", root, 255, 255, 255, true)
				exports.dLogs:createDCLog("**"..getPlayerName(source).. "** nyitott egy Strandlabdát, ezt kapta belőle: **".. winnedItemName .."**.", 1)
			end
		...
		if not targetPlayer then
			...
			if targetPlayer then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 3 and pint == tint and pdim == tdim then
					...
					outputChatBox("#E48F8F► alphaGames: #ffffffA kiválasztott játékos túl messze van tőled!", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
		...
		if isElement(source) and client and client == source then
			if pixels and itemId then
				...
				if placednotes >= 3 then
					exports.dhud:showInfobox(source, "e", "Maximum 3 jegyzetet tűzhetsz ki egyszerre!")
				else
					...
					if slot then
						...
						exports.dhud:showInfobox(source, "s", "Sikeresen kitűzted! Várj egy kicsit, és megjelenik!")
					end
				end
			else
				exports.dhud:showInfobox(source, "e", "A jegyzet kitűzése meghiúsult!")
			end
		end
	end)
	...
	function (text, renameItemId, nameTagItemId)
		if isElement(source) and client and client == source and text and renameItemId and nameTagItemId then
			...
			if success == 2 then
				...
				exports.dhud:showInfobox(source, "e", "Az item átnevezése meghiúsult.")
			end
		end
	end)
	...
	function (sourcePlayer, commandName, targetPlayer, itemId, amount, data1, data2, data3)
		if getElementData(sourcePlayer, "adminLevel") >= 5 then
			...
			--[[if getElementData(sourcePlayer, "adminLevel") <= 7 and itemId == 42 or itemId == 41 then
				return outputChatBox("#E48F8F► alphaGames: #ffffffEzt az itemet nem kécarheted le!", sourcePlayer, 255, 255, 255, true)
			end]]
			...
			if not targetPlayer or not itemId or not amount then
				...
				if targetPlayer then
					...
					if state then
						outputChatBox("#E48F8F► alphaGames: #ffffffA kiválasztott tárgy sikeresen odaadásra került.", sourcePlayer, 0, 0, 0, true)
						exports.dLogs:createDCLog("**"..playerName.. "** addolt egy tárgyat **"..targetName.."**-nak/nek Item: " ..getItemName(itemId).." ("..itemId..") | Darab: **"..amount.."**", 1)
					else
						outputChatBox("#E48F8F► alphaGames: #ffffffA kiválasztott tárgy odaadása meghiúsult.", sourcePlayer, 0, 0, 0, true)
					end
				end
			end
		end
				...
				dbQuery(
					function (query)
						...
						if result and dbID then
							...
							if ownerType == "player" then
								triggerClientEvent(element, "giveItemToClient", element, ownerType, itemsTable[element][slotId])
	...
	function (dbID, itemId, movedSlotId, hoverSlotId, stackAmount, ownerElement, targetElement)
		...
		if ownerElement == targetElement then
			...
			if itemsTable[ownerElement][hoverSlotId] then
				if itemsTable[ownerElement][hoverSlotId].locked then
					outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFEz a slot zárolva van! Kérlek várj egy kicsit.", source, 255, 255, 255, true)
				else
					outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFA kiválasztott slot foglalt.", source, 255, 255, 255, true)
				end
		...
		if targetType == "vehicle" then
			if isVehicleLocked(targetElement) then
				outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFA kiválasztott jármű csomagtartója zárva van.", source, 255, 255, 255, true)
		...
		if isElement(inventoryInUse[targetElement]) then
			outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFNem rakhatsz tárgyat az inventoryba amíg azt valaki más használja!", source, 255, 255, 255, true)
			triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
			return
		end
		...
		if targetType == "object" then
			if storedSafes[targetId] then
				if storedSafes[targetId].ownerGroup > 0 then
					if not exports.dgroups:isPlayerInGroup(source, storedSafes[targetId].ownerGroup) then
						outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFA kiválasztott széfhez nincs kulcsod.", source, 255, 255, 255, true)
						triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
						return
					end
		...
		if targetType ~= "player" then
			if itemId == 361 then -- Pénzkazetta
				outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFEzt az itemet csak más játékosnak adhatod át!", source, 255, 255, 255, true)
				triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
				return
			end
		end
		...
		if not hoverSlotId then
			outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFNincs szabad slot a kiválasztott inventoryban!", source, 255, 255, 255, true)
			triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
			return
		end
		...
		----	outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFA kiválasztott inventory nem bírja el ezt a tárgyat!", source, 255, 255, 255, true)
		--	triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
		--	return false
		--end
		...
		--exports.dLogs:logCommand(source, eventName, {dbID, itemId, stackAmount, ownerType, ownerId, targetType, targetId})
		...
		if ownerType == "player" and targetType == "player" then
			--exports.dchat:localAction(ownerElement, "átadott egy tárgyat " .. getElementData(targetElement, "visibleName"):gsub("_", " ") .. "-nak/nek." .. itemName)
			local targetName = getPlayerName(targetElement)
			local ownerName = getPlayerName(ownerElement)
			...
			exports.dLogs:createDCLog("**"..ownerName.."** átadott **"..stackAmount.."db"..itemName.."** ItemID: **("..itemId..")** Item dbID: **("..dbID..")** tárgyat **"..targetName.."** játékosnak.", 9)
		...
		if isElement(element) then
			if ownerId and ownerType then
				...
				elseif ownerType == "object" then
					if storedSafes[ownerId] then
						...
						if storedSafes[ownerId].ownerGroup > 0 then
							if not exports.dgroups:isPlayerInGroup(source, storedSafes[ownerId].ownerGroup) then
								canOpenInv = false
							end
				...
				if not canOpenInv then
					outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFA kiválasztott inventory zárva van, esetleg nincs kulcsod hozzá.", source, 255, 255, 255, true)
					return
				end

				if isElement(inventoryInUse[element]) then
					outputChatBox("#E48F8F[alphaGames - Inventory]: #FFFFFFA kiválasztott inventory már használatban van!", source, 255, 255, 255, true)
					return
				end
				...
				if ownerType == "vehicle" then
					exports.dchat:localAction(source, "belenézett a csomagtartóba.")
				...
				elseif ownerType == "object" then
					exports.dchat:localAction(source, "belenézett a széfbe.")
				end
			end
		end
	end)
				...
				function (query)
					...
					if result then
						...
						if lost > 0 and ownerType == "player" then
							outputChatBox("#E48F8F[alphaGames - Inventory]: #4aabd0" .. lost .. " #ffffffdarab elveszett tárggyal rendelkezel.", element, 255, 255, 255, true)

							if restored > 0 then
								outputChatBox("#E48F8F[alphaGames - Inventory]: #ffffffEbből #4aabd0" .. restored .. " #ffffffdarab lett visszaállítva.", element, 255, 255, 255, true)
							end

							if lost - restored > 0 then
								outputChatBox("#E48F8F[alphaGames - Inventory]: #ffffffNem sikerült visszaállítani #4aabd0" .. lost - restored .. " #ffffffdarab tárgyad, mert nincs szabad slot az inventorydban.", element, 255, 255, 255, true)
								outputChatBox("#E48F8F[alphaGames - Inventory]: #ffffffkövetkező bejelentkezésedkor ismét megpróbáljuk.", element, 255, 255, 255, true)
							end

							if lost == restored then
								outputChatBox("#E48F8F[alphaGames - Inventory]: #ffffffAz összes hibás tárgyadat sikeresen visszaállítottuk. Kellemes játékot kívánunk! :).", element, 255, 255, 255, true)
							end
						end
					end
